    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-serif font-light text-white">Workspaces</h1>
            <p className="text-violet-300 mt-1">Collaborate on shared signal feeds with your team</p>
          </div>
          <Button>
        ...
        <Card variant="glass" padding="md">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-500 to-violet-700 flex items-center justify-center">
              <UsersIcon className="w-6 h-6 text-white" />
            </div>
            <div className="flex-1">
              <h3 className="font-medium text-white">{workspace.name}</h3>
              <p className="text-violet-400 text-sm">{workspace.description}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-violet-300 text-sm">{workspace.members.length} members</span>
      ...
      <Tabs defaultValue={activeTab} onChange={setActiveTab}>
        <Tabs.List className="mb-6">
          ...
          <Tabs.Trigger value="queues">
            <LinkIcon className="w-4 h-4" /> Shared Queues ({workspace.shared_queues.length})
          </Tabs.Trigger>
          <Tabs.Trigger value="sources">
            <MagnifyingGlassIcon className="w-4 h-4" /> Shared Sources ({workspace.shared_sources.length})
          </Tabs.Trigger>
            ...
            {[
              ...
              <div key={i} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl border border-white/10">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-violet-200 text-sm">{activity}</p>
        ...
        <Card variant="elevated" padding="lg">
          ...
          <div className="space-y-2">
            {Object.entries(workspace.routing_policies).map(([sourceId, policy]) => {
              ...
              return (
                <div key={sourceId} className="flex items-center justify-between p-2 bg-white/5 rounded-xl">
                  <span className="text-sm text-violet-300 truncate pr-2">{source?.title || sourceId}</span>
                  <Badge variant={policy === 'curated' ? 'violet' : 'outline'} size="sm">{policy}</Badge>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card variant="glass" padding="lg">
        ...
        <p className="text-violet-400">No pending invitations.</p>
      </Card>
    </div>
  );
}
    ...
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-violet-400 text-sm">{member.email}</p>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {workspace.shared_queues.map((queue) => (
          <Card key={queue.queue_id} variant="elevated" padding="lg">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h4 className="text-lg font-medium text-white">{queue.name}</h4>
                <p className="text-violet-400 text-sm mt-1">{queue.description}</p>
              </div>
              <Badge variant="violet" size="sm">{queue.routing_policy}</Badge>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {queue.source_ids.map(sourceId => {
                  const source = MOCK_SOURCES.find(s => s.source_id === sourceId);
                  return (
                    <Badge key={sourceId} variant="outline" size="sm" className="truncate max-w-[150px]">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-violet-400 text-sm">{source.publisher} • {source.frequency}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
...
function SettingsTab({ workspace }: { workspace: Workspace }) {
  return (
    <div className="max-w-2xl space-y-8">
      <Card variant="elevated" padding="lg">
        <h3 className="font-medium text-white mb-6">Workspace settings</h3>
        <div className="space-y-6">
          <div>
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
            ...
            <select className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white">
      ...
      <Card variant="elevated" padding="lg" className="border-red-500/20">
        ...
        <p className="text-violet-400 mb-4">Deleting this workspace will remove all shared queues and sources. This action cannot be undone.</p>
...
function StatCard({ label, value, icon }: { label: string; value: number; icon: string }) {
  return (
    <div className="p-4 bg-white/5 rounded-xl border border-white/10">
      ...
      <p className="text-violet-400 text-sm">{label}</p>
    </div>
  );
}
