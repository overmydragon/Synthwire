    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-serif font-light text-white">Archive Search</h1>
            <p className="text-violet-300 mt-1">Full-text search across all indexed issues</p>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="secondary" size="sm" onClick={clearFilters} disabled={activeFilterCount === 0}>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1">
            <MagnifyingGlassIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 text-violet-400" />
            <input
              type="search"
              value={query}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-12 py-4 text-white placeholder-violet-500 text-lg focus:outline-none focus:ring-2 focus:ring-violet-500/50"
            />
          </div>
          <div className="flex items-center gap-3">
            <Button variant="secondary" size="lg" onClick={() => setShowFilters(!showFilters)} className="gap-2">
      ...
      {/* Filters panel */}
      {showFilters && (
        <Card variant="glass" padding="lg" className="mb-6 animate-slide-down">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Source filter */}
            <div>
              <label className="block text-sm font-medium text-violet-300 mb-2">Sources</label>
              <div className="relative">
                <FunnelIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-violet-400" />
                <select
                  multiple
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-10 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50 h-32"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-violet-400" />
                  <input
                    type="date"
                    value={dateRange.start}
                    onChange={(e) => setDateRange(prev => ({ ...prev, start: e.target.value }))}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-10 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50"
                  />
                </div>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="relative">
                <TagIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-violet-400" />
                <select
                  multiple
                  value={topicFilters}
                  onChange={(e) => {
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-white font-mono w-12">{minRelevance}%</span>
              </div>
            </div>
          </div>
        </Card>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <select
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50"
            >
          ...
          {results.length === 0 ? (
            <Card variant="outlined" padding="lg" className="text-center py-16">
              ...
              <p className="text-violet-400">Try adjusting your search query or filters.</p>
            </Card>
          ) : (
...
function SearchResultCard({ result }: { result: SearchResult }) {
  ...
  return (
    <Card variant="default" padding="lg" className="hover:border-violet-500/30 transition-colors">
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-violet-300 text-sm mb-3 line-clamp-2">{result.excerpt}</p>
