...
const SUMMARY_MODES = [
  ...
  { value: 'analyst_brief', label: 'Analyst brief', description: 'Deep analysis (~12 min)', icon: LayersIcon },
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-serif font-light text-white">Digest Builder</h1>
            <p className="text-violet-300 mt-1">Configure and preview your personalized brief</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button size="lg" onClick={generateDigest} className="group">
        ...
        <Card variant="glass" padding="md" className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-4 flex-1 min-w-[300px]">
            ...
            <select
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50"
            >
              {CADENCE_OPTIONS.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
            </select>

            <label className="text-sm font-medium text-violet-300 whitespace-nowrap">Summary mode</label>
            <select
              value={selectedMode}
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {SUMMARY_MODES.map(mode => (
            ...
            >
              <mode.icon className={`w-5 h-5 mb-2 ${selectedMode === mode.value ? 'text-violet-400' : 'text-violet-500'}`} />
              <p className="font-medium text-white">{mode.label}</p>
              <p className="text-violet-400 text-sm mt-1">{mode.description}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {digest.sections.length === 0 && (
        <Card variant="outlined" padding="lg" className="text-center py-16">
          ...
          <Button asChild>
            <a href="/app/discovery">Browse Recommendations</a>
          </Button>
        </Card>
      )}
    </div>
